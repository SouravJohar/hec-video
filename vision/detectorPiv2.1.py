        ...
        for item in items:
            try:
                ...
                if (not skip_cnn and alert == 1) or (not skip_cnn and skip_irr):
                    ...
                    if res:
                        alert = 2
                        cv2.imwrite("out/res_" + str(c) + ".jpg",
            ...
            try:
                ...
                if (not skip_cnn and alert == 1) or (not skip_cnn and skip_irr):
                    ...
                    if res:
                        ...
                        str_current_time = time.strftime("%I:%M:%S %p on %a, %d %b %Y", time.localtime())
                        
                        if last_sent == 0 or (current_time - last_sent >= time_gap_in_hours * 3600) and bar == sensitivity:
                            sms_alert(9786395805, str_current_time)
                            image_alert("out/res_" + str(c) + ".jpg", str_current_time)
                ...
                if _:
                    ...
                    if (not skip_cnn and alert == 1) or (not skip_cnn and skip_irr):
                        ...
                        if res:
                            alert = 2
                            cv2.imwrite("out/res_" + str(c) + ".jpg",
                                        cv2.cvtColor(item, cv2.COLOR_RGB2BGR))
